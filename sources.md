...
This file should evolve as the user's tastes shift and new discovery channels emerge. Work with the user to keep it current:

### When to propose changes
- A recommendation from a new source lands well → suggest adding that source
- An award list consistently produces misses → suggest demoting or removing it
- The user mentions a new genre interest, subreddit, newsletter, podcast, or reviewer they trust → add it
- Seasonal: after major award announcements (Hugo/Nebula season ~April-August, Booker ~Sept-Nov), prompt the user to review this file

### How to refine
1. **Ask before changing** — propose additions/removals with a brief rationale and wait for confirmation
2. **Track hit rate** — if you notice a source consistently surfaces books the user rates highly, note that here (e.g., "Hugo novella shortlist: 4/5 hits")
3. **Add user-specific channels** — blogs, BookTube/BookTok creators, newsletters, subreddits the user follows
4. **Retire stale sources** — if an award hasn't run recently or the user never picks from it, move it to an "Inactive" section rather than deleting

### User-Added Sources
<!-- Add trusted reviewers, newsletters, subreddits, podcasts, etc. here -->

### Inactive / Low-Hit Sources
<!-- Move sources here that haven't been useful recently instead of deleting them -->
