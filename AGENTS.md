...
Two CLI scripts are available for checking availability and placing holds at the user's libraries.
Auth tokens are stored in `.env` (auto-loaded by Bun). If tokens expire, use `/refresh-kcls` or `/refresh-libby` prompts to update them via Playwriter.
...
bun kcls.ts hold S82C2258007                 # Place hold by metadata ID
bun kcls.ts hold S82C2258007 --format DIGITAL
bun kcls.ts cancel <hold_id>                 # Cancel a hold
...
bun libby.ts hold <media_id>                  # Place hold (KCLS default)
bun libby.ts hold <media_id> --card 94349882  # Place hold on SDCL card
